...
import { FolderOpen, Search, Download, Eye, Calendar, User, FileImage, FileText, Filter } from 'lucide-react';

export const DoctorDocuments = () => {
  const { selectedPatientId } = useAuth();
  const { darkMode } = useUI();
  const { medicalDocuments } = useMedical();
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredDocs.length === 0 ? (
          <div className={`col-span-3 text-center py-20 rounded-medical border 
            ${darkMode ? 'bg-slate-900/60 border-slate-800' : 'bg-white border-slate-200'}`}>
            <span className="text-xs text-slate-400">No medical documents uploaded for this patient.</span>
          </div>
        ) : (
            ...
            >
              <div>
                <div className="flex justify-between items-start">
                  ...
                  <span className="text-[9px] font-bold text-slate-550 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded">
                    {doc.type}
                  </span>
                </div>
                ...
                <div className="space-y-2 mt-4 pt-4 border-t border-slate-100 dark:border-slate-850">
                  <div className="flex items-center text-[10px] text-slate-405">
                    <User size={13} className="mr-1.5 text-primary" />
                    <span className="truncate">Uploaded by: <b className="font-semibold text-slate-650 dark:text-slate-350">{doc.uploaderName}</b></span>
                  </div>
                  <div className="flex items-center text-[10px] text-slate-405">
              ...
              {/* Actions */}
              <div className="border-t border-slate-105 dark:border-slate-850/80 pt-3 mt-4 flex items-center justify-between">
                ...
                <div className="flex items-center space-x-1.5">
                  <button
                    ...
                    title="View Document Scan"
                  >
                    <Eye size={14} />
                  </button>
                  <button
                    className="p-1.5 text-slate-450 hover:bg-slate-105 dark:hover:bg-slate-850 rounded-lg transition-colors"
